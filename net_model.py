...
class MPScrModelStructure(keras.models.Model):    
    ...
    def loss_func(self, m, eta):
        ...
        if(self.r_min is not None and self.r_max is not None):          
            ...
            penalty_lower = ( tf.maximum(tf.constant(0.0, dtype = tf.float64), r_min_vector - r0) / (r_min_vector - r0) * (self.r_mid - r0) )**2
            ...
            # Toma max( 0, min_p_0 - p_0 )^2 (eleva ao quadrado para suavizar o gradiente)
            penalty_upper = ( tf.maximum(tf.constant(0.0, dtype = tf.float64), r0 - r_max_vector) / (r0 - r_max_vector) * (r0 - self.r_mid) )**2
