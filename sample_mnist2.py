...
def sample_single_bootstrap_scenario2(n, distribution, file_index, train_labels, test_labels):
    ...
    # ---------------------------- RGP(q = 1/2) ----------------------------
    elif(distribution.lower() == "rgp32"):
        q = 3/2
        # RGP(1/2) - Training data
        theta_train_rgp32 = get_theta(log_a_rgp(q), log_phi_rgp(q), C_rgp(q), C_inv_rgp(q), sup_rgp(q), p_train, theta_min = theta_min_rgp, theta_max = theta_max_rgp(q))
        m_train_rgp32, t_train_rgp32, delta_train_rgp32, cured_train_rgp32 = \
            generate_data(log_a_rgp(q), log_phi_rgp(q), theta_train_rgp32, sup_rgp(q), low_c, high_c)
        # RGP(1/2) - Validation data
        theta_val_rgp32 = get_theta(log_a_rgp(q), log_phi_rgp(q), C_rgp(q), C_inv_rgp(q), sup_rgp(q), p_val, theta_min = theta_min_rgp, theta_max = theta_max_rgp(q))
        m_val_rgp32, t_val_rgp32, delta_val_rgp32, cured_val_rgp32 = \
            generate_data(log_a_rgp(q), log_phi_rgp(q), theta_val_rgp32, sup_rgp(q), low_c, high_c)
        # RGP(1/2) - Test data
        theta_test_rgp32 = get_theta(log_a_rgp(q), log_phi_rgp(q), C_rgp(q), C_inv_rgp(q), sup_rgp(q), p_test, theta_min = theta_min_rgp, theta_max = theta_max_rgp(q))
        m_test_rgp32, t_test_rgp32, delta_test_rgp32, cured_test_rgp32 = \
            generate_data(log_a_rgp(q), log_phi_rgp(q), theta_test_rgp32, sup_rgp(q), low_c, high_c)
        # Save the DataFrame with the simulated values for the RGP(1/2)
        rgp32_data = join_datasets(
            n_train, n_val, n_test,
            theta_train_rgp32, theta_val_rgp32, theta_test_rgp32,
            m_train_rgp32, m_val_rgp32, m_test_rgp32,
            t_train_rgp32, t_val_rgp32, t_test_rgp32,
            delta_train_rgp32, delta_val_rgp32, delta_test_rgp32
        )
        rgp32_data.to_csv("{}/rgp32/{}".format(directory, filename), index = False)
...
if(__name__ == "__main__"):
    ...
    dists_scenario2 = ["borel", "rgp32", "haight", "geeta3"]
    ...
    # print("Sampling RGP(3/2)")
    # np.random.seed(9)
    # sample_bootstrap_scenario2(500, "rgp32", train_labels, test_labels)
    # sample_bootstrap_scenario2(1000, "rgp32", train_labels, test_labels)
    # sample_bootstrap_scenario2(3000, "rgp32", train_labels, test_labels)
